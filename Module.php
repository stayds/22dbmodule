...
class Module
{
    ...
    public function getGames(){
        try{
            $sql = "TRUNCATE TABLE bet22 RESTART IDENTITY CASCADE";
            $this->db->query($sql);
            $table = 'bet22';
            //getRecent($table);
    ...
    private function getGamesOnline(){
        $leagues = getLeagues();
        $table = 'bet22';
    ...
    private function matchId(){
        $sql = "select matchid,corner_id,card_id from bet22";
        $match = $this->db->query($sql);
        $match_array = [];
    ...
    public function m1x2_ht($match,$gid,$standard){
         ...
         insertOutcomes($outcome,$standard,'bet22');
     }
